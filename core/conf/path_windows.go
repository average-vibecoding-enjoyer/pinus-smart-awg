...
const protectedDataDirectorySDDL = "D:PAI(A;OICI;FA;;;SY)(A;OICI;FA;;;BA)"

const protectedDataDirectorySecurityInformation windows.SECURITY_INFORMATION = windows.DACL_SECURITY_INFORMATION | windows.PROTECTED_DACL_SECURITY_INFORMATION
...
func RootDirectory(create bool) (string, error) {
	...
	dataDirectorySd, err := windows.SecurityDescriptorFromString(protectedDataDirectorySDDL)
	if err != nil {
		return "", err
	}
	...
	for {
		...
		dataHandle, err = windows.CreateFile(data16, windows.READ_CONTROL|windows.WRITE_DAC, windows.FILE_SHARE_READ|windows.FILE_SHARE_WRITE|windows.FILE_SHARE_DELETE, nil, windows.OPEN_EXISTING, windows.FILE_FLAG_BACKUP_SEMANTICS|windows.FILE_FLAG_OPEN_REPARSE_POINT|windows.FILE_ATTRIBUTE_DIRECTORY, 0)
	...
	err = windows.SetKernelObjectSecurity(dataHandle, protectedDataDirectorySecurityInformation, dataDirectorySd)
	if err != nil {
		return "", err
	}
	cachedRootDir = data
