...
func TestWintunOrdering(t *testing.T) {
	if os.Getenv("PINUS_RUN_WINTUN_TESTS") != "1" {
		t.Skip("set PINUS_RUN_WINTUN_TESTS=1 to run the privileged packet-ordering stress test")
	}
	...
	go func() {
		...
		for sendingIndex := uint64(0); !doneTunWrite; sendingIndex++ {
			...
			n, err := tunDevice.Write([][]byte{packet[:]}, 0)
			if err != nil {
				fatalErrors <- err
			}
			if n == 0 {
	...
	go func() {
		...
		for i := uint64(0); i < packetsPerTest; {
			...
			sizes := make([]int, 1)
			packetsRead, err := tunDevice.Read([][]byte{buffer[:]}, sizes, 0)
			if err != nil {
				fatalErrors <- err
			}
			if packetsRead == 0 {
