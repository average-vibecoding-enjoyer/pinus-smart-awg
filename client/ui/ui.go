...
func RunUI() {
	...
	for mtw == nil {
		mtw, err = NewManageTunnelsWindow(false)
		if err != nil {
	...
	onUpdateNotification := func(updateState manager.UpdateState) {
		...
		mtw.Synchronize(func() {
			...
			case manager.UpdateStateUpdatesDisabledUnofficialBuild:
				// Updates are intentionally disabled for this MVP fork.
			}
		})
	}
	...
	if shouldQuitManagerWhenExiting {
		...
		if err != nil {
			showErrorCustom(nil, "Error Exiting Pinus Smart AWG", l18n.Sprintf("Unable to exit service due to: %v. You may want to stop Pinus Smart AWG from the service manager.", err))
		}
	}
}
