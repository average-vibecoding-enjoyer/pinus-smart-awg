...
func (tray *Tray) setup() error {
	...
	tray.SetToolTip("Pinus Smart AWG: отключён")
	...
	for _, item := range [...]struct {
		...
		{separator: true},
		{label: "&О Pinus Smart AWG...", handler: tray.onAbout, enabled: true},
...
func (tray *Tray) addTunnelAction(tunnel *manager.Tunnel) {
	...
	tunnelAction.Triggered().Attach(func() {
		...
		go func() {
			oldState, err := tclosure.State()
			if err == nil && oldState == manager.TunnelStarted {
				err = errors.Join(manager.IPCClientSmartStop(), tclosure.Stop())
			} else if err == nil {
				settings, settingsErr := smart.LoadSettings(tclosure.Name)
				if settingsErr != nil {
					err = settingsErr
				} else if smart.NormalizeMode(string(settings.Mode)) == smart.ModeSelected && !hasSelectedVPNRoute(settings) {
					err = fmt.Errorf("в режиме «Только выбранное» включи хотя бы один сервис или правило через VPN")
				} else if payload, payloadErr := smart.SettingsPayload(settings); payloadErr != nil {
					err = payloadErr
				} else {
					err = manager.IPCClientSmartStart(tclosure.Name, payload)
				}
			}
			if err != nil {
				tray.mtw.Synchronize(func() {
					raise(tray.mtw.Handle())
					tray.mtw.dashboard.SelectProfileName(tclosure.Name)
					tray.mtw.dashboard.ShowPage(pageHome)
			...
			if tunnelAction != nil {
				...
				case manager.TunnelStarted:
					if !wasChecked {
						...
						tray.ShowCustom("Pinus Smart AWG подключён", fmt.Sprintf("Профиль %s подключён.", tunnel.Name), icon)
					}

				case manager.TunnelStopped:
					if wasChecked {
						icon, _ := loadSystemIcon("imageres", -31, 128) // TODO: this icon isn't very good...
						tray.ShowCustom("Pinus Smart AWG отключён", fmt.Sprintf("Профиль %s отключён.", tunnel.Name), icon)
					}
				}
			}
		} else if !tray.mtw.Visible() {
			tray.ShowError("Ошибка туннеля Pinus Smart AWG", err.Error())
		}
...
func (tray *Tray) updateGlobalState(globalState manager.TunnelState) {
	...
	tray.SetToolTip("Pinus Smart AWG: " + textForState(globalState, true))
...
func (tray *Tray) UpdateFound() {
	...
	showUpdateTab := func() {
		tray.mtw.dashboard.ShowPage(pageDiagnostics)
		raise(tray.mtw.Handle())
	}
	...
	showUpdateBalloon := func() {
		...
		tray.ShowCustom("Pinus Smart AWG Update Available", "An update to Pinus Smart AWG is now available.", icon)
	}
...
func (tray *Tray) onManageTunnels() {
	tray.mtw.dashboard.SelectActiveProfile()
	tray.mtw.dashboard.ShowPage(pageHome)
	raise(tray.mtw.Handle())
}
...
func (tray *Tray) onImport() {
	raise(tray.mtw.Handle())
	tray.mtw.dashboard.ShowPage(pageProfiles)
	tray.mtw.dashboard.ImportProfiles()
}
