...
func NewTunnelsPage() (*TunnelsPage, error) {
	...
	fillerInfoLabel2.SetText("Import only profiles you trust.")
	...
	infoLabel2.SetText("Import only profiles you trust.")
	...
	tp.updateConfView()
	tp.listView.CurrentIndexChanged().Attach(tp.onCurrentTunnelChanged)
	tp.onTunnelsChanged()
...
func (tp *TunnelsPage) onCurrentTunnelChanged() {
	tp.updateConfView()
	tp.switchCurrentTunnel()
}
...
func (tp *TunnelsPage) switchCurrentTunnel() {
	tunnel := tp.listView.CurrentTunnel()
	if tunnel == nil {
		return
	}
	if !atomic.CompareAndSwapInt32(&tp.switchingProfile, 0, 1) {
		return
	}
	mode := smart.LoadMode(tunnel.Name)
	go func(tunnel manager.Tunnel, mode smart.Mode) {
		defer atomic.StoreInt32(&tp.switchingProfile, 0)
