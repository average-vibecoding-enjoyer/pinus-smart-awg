...
func (sml *smartModeLine) widgets() (walk.Widget, walk.Widget) {
	return sml.label, sml.combo
}

func (sml *smartModeLine) mode() smart.Mode {
	if sml == nil || sml.combo == nil {
		return smart.ModeAll
	}
	return smart.ModeFromIndex(sml.combo.CurrentIndex())
}

func (sml *smartModeLine) setMode(mode smart.Mode) {
	if sml == nil || sml.combo == nil {
		return
	}
	sml.updating = true
	sml.combo.SetCurrentIndex(smart.IndexOfMode(mode))
	sml.updating = false
}

func (sml *smartModeLine) Dispose() {
	sml.label.Dispose()
	sml.combo.Dispose()
}

func newSmartModeLine(parent walk.Container) (*smartModeLine, error) {
	var err error
	var disposables walk.Disposables
	defer disposables.Treat()

	sml := new(smartModeLine)
	if sml.label, err = walk.NewTextLabel(parent); err != nil {
		return nil, err
	}
	disposables.Add(sml.label)
	sml.label.SetText("Routing mode:")
	sml.label.SetTextAlignment(walk.AlignHFarVNear)

	if sml.combo, err = walk.NewComboBox(parent); err != nil {
		return nil, err
	}
	disposables.Add(sml.combo)
	sml.combo.SetModel(smart.ModeLabels())
	sml.combo.SetCurrentIndex(0)
...
func newInterfaceView(parent walk.Container) (*interfaceView, error) {
	...
	if iv.smartMode, err = newSmartModeLine(parent); err != nil {
		return nil, err
	}
	disposables.Add(iv.smartMode)
	...
	iv.lines = append([]widgetsLine{iv.status, iv.smartMode}, append(iv.lines, iv.toggleActive)...)
...
func NewConfView(parent walk.Container) (*ConfView, error) {
	...
	cv.interfaze.smartMode.combo.CurrentIndexChanged().Attach(cv.onSmartModeChanged)
...
func (cv *ConfView) onToggleActiveClicked() {
	...
	go func() {
		var oldState manager.TunnelState
		var err error
		oldState, err = cv.tunnel.State()
		if err == nil && oldState == manager.TunnelStarted {
			err = manager.IPCClientSmartStop()
			_ = cv.tunnel.Stop()
		} else if err == nil {
			_ = cv.tunnel.Stop()
			err = manager.IPCClientSmartStart(cv.tunnel.Name, string(cv.interfaze.smartMode.mode()))
		}
		if err != nil {
			cv.Synchronize(func() {
...
func (cv *ConfView) onSmartModeChanged() {
	if cv.interfaze.smartMode.updating || cv.tunnel == nil {
		return
	}
	mode := cv.interfaze.smartMode.mode()
	if err := smart.SaveMode(cv.tunnel.Name, mode); err != nil {
		showErrorCustom(cv.Form(), "Routing mode", err.Error())
		return
	}
	cv.switchTunnelMode(mode)
}

func (cv *ConfView) switchTunnelMode(mode smart.Mode) {
	tunnel := cv.tunnel
	if tunnel == nil {
		return
	}
	...
	go func() {
		_ = tunnel.Stop()
		err := manager.IPCClientSmartStart(tunnel.Name, string(mode))
		if err != nil {
			cv.Synchronize(func() {
				showErrorCustom(cv.Form(), "Routing mode", err.Error())
			})
...
func (cv *ConfView) setTunnel(tunnel *manager.Tunnel, config *conf.Config, state manager.TunnelState) {
	...
	if tunnel != nil {
		cv.interfaze.smartMode.setMode(smart.LoadMode(tunnel.Name))
	}
