...
func checkForWow64() {
	...
	if b {
		fatalf("You must use the native version of Pinus Smart AWG on this computer.")
	}
}
...
func checkForAdminGroup() {
	...
	if !elevate.TokenIsElevatedOrElevatable(processToken) {
		fatalf("Pinus Smart AWG may only be used by users who are a member of the Builtin %s group.", elevate.AdminGroupName())
	}
}
...
func checkForAdminDesktop() {
	...
	if !adminDesktop && err == nil {
		fatalf("Pinus Smart AWG is running, but the UI is only accessible from desktops of the Builtin %s group.", elevate.AdminGroupName())
	}
}
...
func installManagerService(waitForUI bool) error {
	if waitForUI {
	...
	return errors.New("Pinus Smart AWG system tray icon did not appear after 30 seconds")
}
...
func main() {
	...
	switch os.Args[1] {
	case "/installmanagerservice", "/installmanagerservicequiet":
		if len(os.Args) != 2 {
			usage()
		}
		waitForUI := os.Args[1] == "/installmanagerservice"
		if err := installManagerService(waitForUI); err != nil {
	...
	case "/uninstallmanagerservice":
		...
		return
	case "/uninstallallservices":
		if len(os.Args) != 2 {
			usage()
		}
		if err := manager.UninstallAllServices(); err != nil {
			fatal(err)
		}
		return
	case "/managerservice":
